...
#    $cmd = sprintf 'junction2gene.pl -junctions %s -preserve -force %d', $out_shift, $FORCE;
    $cmd = sprintf '%s -junctions %s -preserve -force %d', $NO_CONFIG_MODE ? "junction2gene_flat.pl" : "junction2gene.pl", $out_shift, $FORCE;
    ...
    if ($genome) {
      ...
      # config system is not involved.  Assume human in this case (HACK)
    } else {
      die;
    }
...
sub init_regions {
  my $ga;
  if ($ger) {
    require GeneAnnotation;
    # put here rather than in "use" to avoid DBI.pm which
    # seems to cause problems with Docker on DNAnexus

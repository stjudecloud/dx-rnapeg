package StructVarCicero;

# $Id: StructVar.pm, v 1.00 2010/09/24 jianmin.wang Exp $

use strict;
use Bio::DB::Sam;
use Statistics::FET qw/ fishers_exact / ;
use Carp;
use Data::Dumper;
use English;
use Bio::SearchIO;
use GeneModelCicero;
use Cwd;
use SVExtToolsCicero;
use List::MoreUtils qw/ uniq /;
use SVUtilCicero qw(read_fa_file get_sclip_reads rev_comp);
use SCValidatorCicero qw(LEFT_CLIP RIGHT_CLIP);
...
my $max_bp_dist = 15;
my $germline_seq_width = 150;
my $germline_search_width = 100;
my $min_percent_cons_of_read = 0.75;
my $max_germline_percent = 0;
...
sub new {
	...
	$self->{junc_seq} = $param{-JUNC_SEQ} ? $param{-JUNC_SEQ} : "";
	$self->{gap} = $param{-GAP} ? $param{-GAP} : "";
	$self->{'insert'} =  $self->{'insert2'} = ' ';
	$self->{'microhomology'} =  $self->{'microhomology2'} = ' ';
	$self->{type} = "";
...
sub add_RNASeq_filter {
	...
	$default_filters{'RNASeq_DEL'}	= \&_RNASeq_DEL_filter;
}
...
sub max_germline_percent {
	my $self = shift;
	my $value = shift;
	$max_germline_percent = $value if($value);
	return $max_germline_percent;
}
...
sub to_unfiltered_string {
	...
	my $rtn = join("\t", ($chrA, $posA, $ortA, $countA, $chrB, $posB, $ortB, $countB,
		$type2str{$type}, $cover1, $cover2, $left_len, $right_len, $self->get_statistics(100), $self->{consensus}));
	return $rtn;
}
...
sub to_string {
	...
	my $rtn = join("\t", ($chrA, $posA, $ortA, $countA, $chrB, $posB, $ortB, $countB,
		...
		$self->{c_end}, $self->{end_chr}, $self->{t_end}, $self->{consensus}, $self->{'insert'}, $self->{'microhomology'}));
	if($self->{type} == INV) {
		$rtn = join("\t", ($rtn, $self->{c_start2}, $self->{start_chr2}, $self->{t_start2},
			$self->{c_end2}, $self->{end_chr2}, $self->{t_end}, $self->{consensus2},  $self->{'insert2'}, $self->{'microhomology2'}));
	}
	return $rtn;
}
...
sub set_consensus {
	...
	if($self->{type} == INV) {
		...
		my $posA = $bp1->{pos};
		my $posB = $bp2->{pos};
		print "\nset consensus for bp1: $posA and bp2: $posB\n";
		$self->{consensus} = _generate_consensus(\@s_reads1);
		my $yyyy=$self->{consensus};
		print "is this consensus seq?\t$yyyy\n";
	}
}
...
#		system ("rm $fa_name"); system("rm $fa_name*");
	...
	my $xxxOut = $contig_seqs->{$contig_name};
	print "consensus contig_seqs\t$contig_name\t$xxxOut\n";
...
sub filter {
	...
	if(!$self->{consensus}  || length $self->{consensus} < $read_len * $min_percent_cons_of_read) {
		print "no consensus\n" if(!$self->{consensus});
		if($self->{consensus}){
			my $xxx = $self->{consensus};
			print "xxxn\t*$xxx* < $read_len * $min_percent_cons_of_read\n" if($self->{consensus});
		}
		$self->{error} = "No Consensus or consensus too short";
		print STDERR "FAILED: No Consensus or consensus too short\n";
		return;
	}
...
sub _mapping_quality {
	...
	if($s1 < $s2) {
		if($e1 < $s2 - 1) {
			$self->{"insert" . $which} = substr($con, $e1, $s2 - $e1 - 1);
		}
		...
		$self->{"microhomology" . $which } = substr($con, $s2-1, $e1 - $s2 + 1)  if($e1 >= $s2);
		...
		if($e2 < $s1 - 1) {
			$self->{"insert" . $which} = substr($con, $e2, $s1 - $e2 - 1);
		}
		...
		$self->{"microhomology" . $which} = substr($con, $s1-1, $e2 - $s1 + 1)  if($e2 >= $s1);
		$s2 = 1;
		($e2, $s1) = ($s1, $e2);
		$e1 = $l;	
	}
	...
	foreach my $r( ($range1, $range2) ) {
		...
		my $percent = $matches[0]/($qend - $qstart + 1);
		print "qstart:\t$qstart\tqend:\t$qend\tmatches:\t@matches\npercent:\t$percent\n";
		$hit = $mapper->run(-QUERY => "query.fa");
		foreach $h (@{$hit->{query}}) {
			if($h->{tchr} eq $chr && $h->{tstart} >= $s - $l && $h->{tend} <= $e + $l) {
			print "if($h->{tchr} eq $chr && $h->{tstart} >= $s - $l && $h->{tend} <= $e + $l)\n";
				print "!!!!!! within ONE gene !!!!!!\n";
			} # the same one
			...
			return if($h->{matches} > $matches[0] && $h->{percent} > $percent); #find a better or as good as match
...
sub _map_consensus {
	...
	my $hsp = $h->next_hsp;
	if($hsp->gaps > 3) {
		my $tmp = $aligner->{OPTIONS};
		$aligner->{OPTIONS} = $tmp . " -fastMap";
		$hits = $aligner->run(-TARGET => $genome . ":" . $range, -QUERY => "query.fa");
		$aligner->{OPTIONS} = $tmp;
		($chr, $s, $e) = split /[:|-]/, $range;
		$h = $hits->{query};
		return unless $h; # can't find a good enough match
...
sub _find_bp_ref_range {
	...
	if($self->{type} == INS && abs($bp->{left_pos} - $bp->{right_pos}) < $ext) {
		$ext = int(($bp->{left_pos} - $bp->{right_pos})/2);	
		$ext = $ext * 2 if($l > $ext );
	}
	$l = 0;
...
sub _germline_sclip_filter {
	...
	my $count =  _compare_seq($self->{first_bp}) +	_compare_seq($self->{second_bp});
#	my $rtn = _compare_seq($self->{first_bp}) &&
#		_compare_seq($self->{second_bp});
	my $c = 0;
	my ($coverage) = $sam_g->features(
		-type => 'coverage', 
		-seq_id => $self->{first_bp}{left_pos} == $self->{first_bp}{pos} ? $self->{first_bp}{left_chr} : $self->{first_bp}{right_chr},
		-start => $self->{first_bp}{pos}, 
		-end => $self->{first_bp}{pos});
	my @t = $coverage->coverage;
	$c += $t[0];
	($coverage) = $sam_g->features(
		-type => 'coverage', 
		-seq_id => $self->{second_bp}{left_pos} == $self->{second_bp}{pos} ? $self->{second_bp}{left_chr} : $self->{second_bp}{right_chr},
		-start => $self->{second_bp}{pos}, 
		-end => $self->{second_bp}{pos});
	@t = $coverage->coverage;
	$c += $t[0];	
	return 1 if $count == 0; # no s-cliped reads in germline
	if($count * 100 <= $c * $max_germline_percent) {
		my $d_cover = $self->{first_bp}{cover} + $self->{second_bp}{cover};
		my $d_sclip = $self->{first_bp}{sc_reads} + $self->{second_bp}{sc_reads};
		my $p_value = fishers_exact($c-$count, $count, $d_cover - $d_sclip,  $d_sclip);
		print join("\t", ($count, $c-$count, $d_sclip, $d_cover - $d_sclip, $p_value));
		print "\n";
		if($p_value < 0.05) {
			$self->{error} = undef;
			return 1;
...
sub _type_distance_filter {
	...
	print STDERR "Type distance filter xxxxxxxx\n";
	...
	if($type == INS || $type == DEL) {
		print STDERR "$bp1->{left_pos}\t$bp2->{left_pos}\t$bp1->{right_pos}\t$bp2->{right_pos}\n";
		($d1, $d2) = ($bp1->{left_pos} - $bp2->{left_pos}, 
			$bp1->{right_pos} - $bp2->{right_pos});
		print STDERR "($d1, $d2) = ($bp1->{left_pos} - $bp2->{left_pos}\n";
	}
	if( $type == INV ){
		($d1, $d2) = ($bp1->{left_pos} - $bp2->{left_pos}, 
	...
	if(abs($d1) > $max_bp_dist || abs($d2) > $max_bp_dist){
		print STDERR "if(abs($d1) > $max_bp_dist || abs($d2) > $max_bp_dist)\n";
		print STDERR "Distance fail\n";
		return;
	}
	print STDERR "($d1, $d2) = ($bp1->{left_pos}\t$bp2->{right_pos}\n$bp2->{left_pos}\t$bp1->{right_pos})\n";
	$self->{error} = undef;
...
sub _tandem_repeat_filter {
	...
	system("/nfs_exports/apps/gnu-apps/NextGen/jzbin/ptrfinder -seq query.fa -repsize $tr_min_size,$tr_max_size -minrep $tr_min_num > query.fa.rep");
	if(-e "query.fa.rep") {
...
sub _RNASeq_INS_filter {
	...
	$chr = "chr" . $chr unless $chr =~ m/chr/;
	my ($r_tree, $f_tree) = ($gm->sub_model($chr, "-"), 	$gm->sub_model($chr, "+"));
	print "RNAseq INS $chr\t$pos1,$pos2\n";
	foreach my $tree( ($r_tree, $f_tree) ) {
		my @tmp;
		@tmp = $tree->intersect([$pos1, $pos2]);
		foreach my $g (@tmp) {
